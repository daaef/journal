<x-layouts.layout>
    ...
    <div class="grid lg:grid-cols-[400px_1fr] gap-4 w-full">
        <div class="grid items-start gap-4">
            <a class="font-bold text-gray-100 flex gap-4 justify-center items-center rounded-[8px] py-1 px-6 bg-primary-500" href="{{ route('download-journal', $journal->uuid) }}">
            ...
            @if (checkJournalInMyCollection($journal->id, auth()->user()->id))
                <form action="{{ route('journals.remove-from-collection') }}" method="post">
                    ...
                    <button name="remove_from_collection" value="remove_from_collection"
                            class="text-gray-100 w-full flex justify-center bg-primary-500 rounded-[8px] px-4 py-1 font-bold hover:bg-primary-600">Remove
            ...
            @else
                <form action="{{ route('journals.add-to-collection') }}" method="post">
                    ...
                    <button name="add_to_collection" value="add_to_collection"
                            class="text-gray-100 w-full flex justify-center bg-primary-500 rounded-[8px] px-4 py-1 font-bold hover:bg-primary-600">Add
