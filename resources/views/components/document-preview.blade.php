...
@php
    $componentId = 'doc-preview-' . uniqid();
@endphp

<div class="document-preview-wrapper" data-document-url="{{ $documentUrl }}" data-document-type="{{ $documentType }}" data-component-id="{{ $componentId }}">
    <!-- Preview Container -->
    <div id="{{ $componentId }}-content" style="width: 100%; height: {{ $height }}; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; background: #f5f5f5;">
        <!-- Loading State -->
        ...
        <div id="{{ $componentId }}-error" class="hidden flex items-center justify-center h-full">
            <div class="text-center p-8">
                ...
                @if($showControls)
                <button onclick="retryDocumentPreview('{{ $componentId }}')" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700">
    ...
    <div class="mt-4 text-center">
        <div class="flex justify-center space-x-3">
            <a id="{{ $componentId }}-open-tab" href="{{ $documentUrl }}" target="_blank" class="inline-flex items-center px-3 py-2 text-sm font-medium text-blue-600 hover:text-blue-800">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                </svg>
                Open in New Tab
            </a>
            <a id="{{ $componentId }}-download" href="{{ $documentUrl }}?download=1" class="inline-flex items-center px-3 py-2 text-sm font-medium text-green-600 hover:text-green-800">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function loadDocumentPreview(wrapper) {
    ...
    .then(response => {
            ...
            return fetch(documentUrl, {
                ...
                if (getContentType && getContentType.includes('application/json')) {
                    return getResponse.json().then(data => {
                        ...
                        } else if (data.success && data.type === 'html' && data.html) {
                            // Direct HTML content
                            displayEl.innerHTML = `
        ...
        // For other URLs, use content-type to determine handling
        if (contentType && contentType.includes('application/pdf')) {
            loadPdfPreview(documentUrl, displayEl, loadingEl, errorEl, componentId);
        } else {
            // Default to pandoc preview for other types
