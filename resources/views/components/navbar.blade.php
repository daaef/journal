<header class="flex flex-wrap sm:justify-start h-[80px] sm:flex-nowrap w-full fixed z-[20000] bg-white text-sm py-3">
    <nav class="container w-full mx-auto px-4 flex flex-wrap basis-full items-center justify-between relative">
        <div class="flex gap-5 items-center">
            ...
            <div id="hs-navbar-alignment"
                ...
                <div class="flex flex-col gap-5 mt-5 lg:flex-row lg:items-center lg:mt-0 lg:ps-5">
                    ...
                    {{--                        href="#">About JAPR</a>--}}
                    <a class="hover:text-gray-400 focus:outline-none focus:text-gray-400 @if (Route::is('editorial')) text-primary-500 font-bold @else text-gray-900 font-medium @endif"
                       href="{{ route('editorial') }}">Editorial Board</a>
                    ...
                    @auth()
                        ...
                        @if (auth()->user()->hasRole('Managing Editor') || auth()->user()->hasRole('Editor in Chief'))
                            <a class="font-bold lg:hidden text-gray-900 hover:text-gray-400 focus:outline-none focus:text-gray-400 "
        ...
        <div class="flex items-center gap-x-2">
            @auth
                <a class="py-2 hidden px-6 lg:inline-flex items-center gap-x-2 text-sm font-medium rounded-[15px] border border-transparent bg-secondary-800 text-white hover:bg-primary-900 focus:outline-none mr-4 focus:bg-secondary-950 disabled:opacity-50 disabled:pointer-events-none"
                   href="{{ route('submit-manuscript') }}">
                    Submit Manuscript
                </a>
                <div class="hs-dropdown pr-[40px] [--strategy:static] sm:[--strategy:fixed] [--adaptive:none] ">
                    ...
                    <div
                        ...
                        @if (auth()->user()->hasRole('Admin'))
                           ...
                           {{-- <a class="@if (Route::is('admin.dashboard.settings')) text-primary-500 font-bold @else text-gray-800 font-medium @endif flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                               href="{{ route('admin.dashboard.settings', auth()->user()->uuid) }}">
                                Profile
                            </a>--}}
                        @endif
                        @if (auth()->user()->hasRole('Editor in Chief') || auth()->user()->hasRole('Managing Editor'))
                           ...
                           {{-- <a class="@if (Route::is('editor.dashboard.settings')) text-primary-500 font-bold @else text-gray-800 font-medium @endif flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                               href="{{ route('editor.dashboard.settings', auth()->user()->uuid) }}">
                                Profile
                            </a>--}}
                        @endif
                        @if (auth()->user()->hasRole('Author'))
                        ...
                        @if (auth()->user()->hasRole('Associate Editor') || auth()->user()->hasRole('Desk Editor'))
                            ...
                            {{--<a class="@if (Route::is('reviewer.dashboard.settings')) text-primary-500 font-bold @else text-gray-800 font-medium @endif flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                               href="{{ route('reviewer.dashboard.settings', auth()->user()->uuid) }}">
                                Profile
                            </a>--}}
                        @endif
                    </div>
